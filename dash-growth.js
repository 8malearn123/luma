...
(function(){
  ...
  function areaChart(data, max, opts={}){
    ...
    let grid='';
    for(let g=0;g<=4;g++){const y=pad.t+ih-(g/4)*ih;grid+=`<line x1="${pad.l}" y1="${y}" x2="${W-pad.r}" y2="${y}" stroke="rgba(201,167,94,.14)" stroke-width="1" stroke-dasharray="2 4"/><text x="${pad.l-8}" y="${y+4}" fill="#5E594F" font-size="10" font-family="IBM Plex Mono" text-anchor="end">${((g/4)*max).toFixed(0)}</text>`;}
    const dots=pts.map((p,i)=>`<circle cx="${p[0].toFixed(1)}" cy="${p[1].toFixed(1)}" r="${i===pts.length-1?4:2.5}" fill="${i===pts.length-1?'#E9D29A':'#C9A75E'}"/>`).join('');
    ...
    return `<svg viewBox="0 0 ${W} ${H}" width="100%" style="overflow:visible">
      <defs><linearGradient id="ag" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#C9A75E" stop-opacity="0.34"/><stop offset="1" stop-color="#C9A75E" stop-opacity="0"/></linearGradient></defs>
      ${grid}<path d="${area}" fill="url(#ag)"/><path d="${line}" fill="none" stroke="#E7D2A0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>${dots}${labels}
    </svg>`;
  }
...
<style>
  ...
  .payout{background:linear-gradient(150deg,#1A1820,#1a140e);border:1px solid var(--gold-deep);border-radius:16px;padding:22px;}
  ...
  .tx-row .av{width:36px;height:36px;border-radius:50%;background:var(--surface3);border:1px solid var(--line);display:flex;align-items:center;justify-content:center;font-family:'Bodoni Moda',serif;color:var(--gold-light);font-size:15px;}
...
<div class="card" style="margin-top:18px">
  ...
  const SRC=[['المتجر',46,'#E7D2A0'],['رابط مباشر',28,'#C9A75E'],['انستقرام',18,'#8A6E34'],['الإحالات',8,'#5a4624']];
...
<style>
  ...
  .peak .col .b{width:100%;max-width:34px;border-radius:6px 6px 0 0;background:linear-gradient(180deg,#E7D2A0,#8A6E34);}
  ...
  .top-svc .bar span{display:block;height:100%;background:linear-gradient(90deg,#C9A75E,#E9D29A);border-radius:10px;}
...
<div class="an-row2">
  ...
  window.EXPLOY={
    ...
    addReward(){
      ...
      {onMount(ov,close){
        ...
        ov.querySelector('[data-ok]').onclick=()=>{
          const nm=ov.querySelector('[name=rn]');if(!nm.value.trim()){nm.style.borderColor='#C97A6A';nm.focus();return;}
          const cfg=eloyCfg();
    ...
    redeem(name){
      ...
      LUX.modal('استبدال نقاط — '+name,`
        ...
        ${avail.map((r,i)=>`<label style="display:flex;align-items:center;gap:11px;border:1px solid var(--line);border-radius:11px;padding:13px 15px;margin-bottom:9px;cursor:pointer"><input type="radio" name="rw" value="${cfg.rewards.indexOf(r)}" ${i===0?'checked':''} style="accent-color:#E7D2A0"/><span style="flex:1;font-size:14px;color:var(--white)">${r.n}</span><span style="font-family:'IBM Plex Mono',monospace;font-size:12px;color:var(--gold-pale);direction:ltr">${r.pts} pts</span></label>`).join('')}
  ...
  S.loyalty = { render(){
    ...
    return `
<style>
  .ly-hero{display:flex;align-items:center;justify-content:space-between;background:linear-gradient(135deg,#1A1820,#1a140e);border:1px solid var(--gold-deep);border-radius:16px;padding:22px 26px;margin-bottom:22px;flex-wrap:wrap;gap:16px;}
  .ly-hero .l{display:flex;align-items:center;gap:16px;}
  .ly-hero .ic{width:52px;height:52px;border-radius:14px;background:rgba(201,167,94,0.2);border:1px solid var(--gold-deep);display:flex;align-items:center;justify-content:center;color:var(--gold-light);}
  ...
  .mem .av{width:36px;height:36px;border-radius:50%;background:var(--surface3);border:1px solid var(--line);display:flex;align-items:center;justify-content:center;font-family:'Bodoni Moda',serif;color:var(--gold-light);font-size:15px;}
...
<style>
  ...
  .ref-row .av{width:36px;height:36px;border-radius:50%;background:var(--surface3);border:1px solid var(--line);display:flex;align-items:center;justify-content:center;font-family:'Bodoni Moda',serif;color:var(--gold-light);font-size:15px;}
...
<div class="card" style="margin-bottom:22px;background:linear-gradient(135deg,#1A1820,#1a140e);border-color:var(--gold-deep)">
  <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px"><div style="width:48px;height:48px;border-radius:13px;background:rgba(201,167,94,0.2);border:1px solid var(--gold-deep);display:flex;align-items:center;justify-content:center;color:var(--gold-light)">${icon('affiliate',22)}</div><div><div style="font-family:'IBM Plex Sans Arabic',Cairo;font-weight:700;font-size:18px;color:var(--white)">ادعي خبيرات أخريات، واكسبي</div><div style="font-size:13px;color:var(--gold-pale);margin-top:2px">احصلي على ١٠٪ من اشتراك الشهر الأول لكل خبيرة تنضم برابطك</div></div></div>
  ...
  window.LUX_shareRef=function(){
    ...
    if(navigator.share){navigator.share(data).then(()=>{window.LUX&&window.LUX.toast('تمت المشاركة ✓','ok');}).catch(()=>{});}
      ...
      window.LUX.modal('مشاركة رابط الدعوة',`<div class="lux-lead">شاركي رابطك مع خبيرات أخريات — تحصلين على مكافأة عند انضمام كل واحدة.</div>
        <div class="lux-row"><span class="k">الرابط</span><span class="v" style="direction:ltr;font-family:'IBM Plex Mono',monospace;color:var(--gold-light,#E7D2A0)">${url}</span></div>
