  ...
  <div style="display:grid;grid-template-columns:1.5fr 1fr;gap:18px;align-items:start">
    <div>
      ...
      <div class="card" style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:12px;padding:13px 16px;flex-wrap:wrap">
        ...
        <button class="btn btn-gold" style="padding:9px 16px" onclick="LUX.toast('استوديو تصميم البطاقات ضمن المرحلة القادمة','ok')">+ تصميم بطاقة</button>
      </div>
    </div>
...
const PAGE_SUBNAV=[['general','عام'],['visits','الزيارات'],['svc','الخدمات'],['prod','المنتجات'],['books','الحجوزات'],['team','الفريق'],['identity','الهوية'],['design','تصميم المتجر'],['front','الواجهة'],['banners','البنرات'],['pages','الصفحات'],['cats','التصنيفات'],['home','التوصيل والمنزل']];
document.getElementById('sb-nav').innerHTML=NAV.map(it=>{
  ...
  return `<div class="nav-item" data-id="${it.id}" onclick="SALON.go('${it.id}')"><span class="ico">${icon(it.icon,20)}</span><span class="lbl">${it.label}</span><span class="dot"></span></div>`;
}).join('');
window.togglePageMenu=function(){
  const sub=document.getElementById('pgSub'),car=document.getElementById('pgCaret');
  const open=sub.style.display!=='none';
  if(open){sub.style.display='none';if(car)car.style.transform='';}
  else{sub.style.display='block';if(car)car.style.transform='rotate(180deg)';
    if((location.hash||'').replace('#','')!=='page')window.PAGE&&PAGE.showTab('general');}
};
